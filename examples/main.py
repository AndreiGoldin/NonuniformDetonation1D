...
def simulate(config_file='config.yaml'):
    ...
    if callbacks["plot final solution"]:
        Writer.plot_solution(mesh, solver.phys_solution, time[-1], solution_savename, dpi=300)

    if callbacks["write final solution"]:
        Writer.write_solution(mesh.nodes[mesh.domain], solver.phys_solution, solution_savename)
