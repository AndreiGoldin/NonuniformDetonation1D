# Contains possible initial conditions
import numpy as np
from scipy.integrate import solve_ivp

def _initial_znd(nodes, params):
    """ Evaluate the Zeldovich-von Neumann-Doring solution at the nodes, that should be on [-L,0] for some L"""
...
def initial_znd(nodes, params):
    znd_part = _initial_znd(nodes[nodes<1e-16], params)
    return znd_part


def initial_znd_lfor(nodes, params):
    """Put ZND solution on [a, 0] and uniform state on [0, b], where nodes are on [a,b]"""
    znd_part = _initial_znd(nodes[nodes<1e-16], params)
