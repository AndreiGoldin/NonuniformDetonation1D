...
import numpy as np
import math
from scipy import signal, interpolate, integrate # for averages and for PSD of detonation speed
...
def variable_transform_jit(array, heat_release, gamma, toConserved):
    ...
    if toConserved:  # from rho,u, p, lam to rho,m,E,Y
        ...
        # Energy
        transformed_array[2, :] = array[2, :] /             (gamma-1)+0.5*transformed_array[1, :] *             transformed_array[1, :]/array[0, :] -             heat_release*transformed_array[3, :]

    else:  # from rho, m, E, Y to rho,u,p,lam
        transformed_array[1, :] = array[1, :]/array[0, :]
        transformed_array[3, :] = array[3, :]/array[0, :]
        # Pressure
        transformed_array[2, :] = (gamma-1)*(array[2, :]-0.5*array[1, :]
                                             * array[1, :]/array[0, :] + heat_release*array[3, :])
...
def periodic_friction(nodes, displacement,eps,k_for_fric):
    ...
    period_part = np.where(nodes > -displacement, (1 + eps*np.sin(k_for_fric*(nodes+displacement))), np.zeros_like(nodes))
    return period_part
...
def D_RHS_approx_jit(gamma, D, step, exact_fluxes, cur_state, cur_state_der, array,nodes,displacement,c_f,eps, k_for_fric):
        """
        ...
        -------
            Value of the RHS
        """
        coef_friction = c_f*periodic_friction(nodes,displacement,eps, k_for_fric)
        source_term_momentum = coef_friction[-3]*array[1, -3]*array[1, -3]/array[0,-3]/2
        #print(source_term_momentum)
        # The momentum flux gradient on the shock
        rho_a, u_a, p_a, lambda_a = cur_state[0], cur_state[1], cur_state[2], cur_state[3]
        m_flux = exact_fluxes # HERE SHOULD BE ONLY MOMENTUM FLUX
        m_flux_dx = -12.*m_flux[0] + 75.*m_flux[1] - 200.*m_flux[2]+                    300.*m_flux[3] - 300.*m_flux[4] + 137.*m_flux[5]
        m_flux_dx /= 60.*step
        vel_dif = D-u_a
        nom = rho_a*vel_dif*(gamma*(rho_a*u_a*vel_dif - 2.*p_a) +
                             rho_a*(2.*D**2-3.*D*u_a+u_a**2))
        denom = gamma*(2.*p_a+rho_a*vel_dif**2)-rho_a*vel_dif**2
        nom_by_denom_sq = nom/denom**2
        # The derivative of the momentum at the shock \rho*u|_s w.r.t. the shock velocity D
        nom_der = nom/vel_dif + rho_a*vel_dif *             (gamma*rho_a*u_a+rho_a*(4.*D-3.*u_a))
        denom_der = 2.*rho_a*(gamma-1.)*vel_dif
        dD_dm = 1./(nom_der/denom - denom_der*nom_by_denom_sq)
        # The derivative of the momentum at the shock \rho*u|_s w.r.t. the laboratory coordinate \xi
        dm_dxi = 0
        if cur_state_der[0] >= 1e-8:
            nom_der = nom/rho_a+rho_a*vel_dif *(gamma*(u_a*vel_dif) + (2.*D**2-3.*D*u_a+u_a**2))
            denom_der = (gamma - 1.)*vel_dif**2
            dm_drho = nom_der/denom - denom_der*nom_by_denom_sq
            dm_dxi += dm_drho*cur_state_der[0]
        if cur_state_der[1] >= 1e-8:
            nom_der =  -nom/vel_dif + rho_a**2*vel_dif *((gamma-3.)*D - (gamma-1.)*2.*u_a)
            denom_der = 2.*(1.-gamma)*rho_a*vel_dif
            dm_du = nom_der/denom - denom_der*nom_by_denom_sq
            dm_dxi += dm_du*cur_state_der[1]
        if cur_state_der[2] >= 1e-8:
            nom_der = -2.*rho_a*vel_dif*gamma
            denom_der = 2.*gamma
...
@nb.njit('Tuple((f8[:,:], f8[:]))(f8[:,:], f8[:], f8, f8, f8[:], f8, f8, f8, f8)')
def Det_source_jit(array, P, act_energy, rate_const, nodes, displacement, c_f, eps, k_for_fric):
            """
            ...
            source[-1, :] = rate_const * (array[0, :] - array[3, :])*                 mexp_jit(-act_energy*array[0, :]/P)
...
@nb.njit('Tuple((f8[:,:], f8[:,:],f8[:,:], f8,f8[:]))(f8[:,:], f8[:], f8, f8, f8, f8, f8,f8, f8[:],f8,f8,f8,f8)')
def Det_LF_jit(array, heat_release, step, D, gamma, act_energy, rate_const, CFL, nodes,displacement,c_f, eps, k_for_fric):
        ...
        u = array[1, :]/array[0, :]
        lam = array[3, :]/array[0, :]
        P = (gamma - 1.)*(array[2, :]-0.5 *
                          array[1, :]*array[1, :]/array[0, :] + array[3, :]*heat_release)
        H = (array[2, :]+P)/array[0, :]
        c = np.sqrt((gamma - 1.)*(H - .5*u*u))
        ...
        RHS, c_fric = Det_source_jit(array, P, act_energy, rate_const, nodes,displacement,c_f, eps,k_for_fric)
...
def Det_BC_WENO5_jit(array, ambient, heat_release, D_next, gamma, beg, end):
    ...
    M_next = D_next/c_a
    p_s = (2.*gamma/(gamma+1.)*M_next*M_next -
           (gamma-1.)/(gamma+1.))*p_a
    rho_s = ((gamma+1.)*M_next*M_next /
             (2.+(gamma-1.)*M_next*M_next))*rho_a
    u_s = (2./(gamma+1.)*(M_next*M_next-1.)/(M_next))*c_a
    lambda_s = lambda_a
    ...
    # Energy
    shock_state_cons[2] = p_s/(gamma-1)+0.5*shock_state_cons[1] *         shock_state_cons[1]/rho_s -         heat_release[-4]*shock_state_cons[3]
...
def WENO_flux_interpolation_jit(nodei_2, nodei_1, nodei, nodeip1, nodeip2):
    ...
    q_im1 =  1./3.*nodei_2 - 7./6.*nodei_1 + 11./6.*nodei
    q_i   = -1./6.*nodei_1 + 5./6.*nodei   +  1./3.*nodeip1
    q_ip1 =  1./3.*nodei   + 5./6.*nodeip1 -  1./6.*nodeip2
    # Indicators of smoothness
    IS_im1 = 13./12.*(nodei_2-2.*nodei_1+   nodei)**2 +                  .25*(nodei_2-4.*nodei_1+3.*nodei)**2
    IS_i   = 13./12.*(nodei_1-2.*nodei+nodeip1)**2 +                  .25*(nodei_1         -nodeip1)**2
    IS_ip1 = 13./12.*(   nodei-2.*nodeip1+nodeip2)**2 +                  .25*(3.*nodei-4.*nodeip1+nodeip2)**2
    # Weights for stencils
    alpha_im1 = 1./10./(eps_forWENO + IS_im1)**2
    alpha_i   = 6./10./(eps_forWENO + IS_i)**2
    alpha_ip1 = 3./10./(eps_forWENO + IS_ip1)**2
    alpha_sum = alpha_im1 + alpha_i + alpha_ip1
    # Modified weights
    mod_omega_0 = g_numba(1./10.,alpha_im1/alpha_sum)
    mod_omega_1 = g_numba(6./10.,alpha_i/alpha_sum)
    mod_omega_2 = g_numba(3./10.,alpha_ip1/alpha_sum)
...
@nb.njit('Tuple((f8[:,:], f8, f8,f8[:]))(f8[:,:], f8[:], f8[:],f8[:], f8, f8, f8, f8, f8, f8, i4,i4,i4,f8[:],f8,f8,f8,f8)')
def spatial_DiffOp_jit(array, heat_release, ambient, ambient_der, step,
                       D, gamma, act_energy, rate_const, CFL, start, finish, end, nodes,displacement,c_f,eps,k_for_fric):
    """
    ...
    """
    ex_fluxes, Evals, RHS, tau,c_fric = Det_LF_jit(array, heat_release, step, D, gamma, act_energy, rate_const, CFL,
                                                   nodes,displacement, c_f, eps, k_for_fric)
    fluxes = ex_fluxes[:, :-1]
    ...
    WENO5M_pos = WENO_flux_interpolation_jit(nodei_2, nodei_1, nodei, nodeip1, nodeip2)
    ...
    WENO5M_neg = WENO_flux_interpolation_jit(nodei_2, nodei_1, nodei, nodeip1, nodeip2)
    # Final numerical flux
    num_flux =  .5*(WENO5M_pos+WENO5M_neg)
    # Numerical conservative flux derivative
    flux_der_for_domain = (num_flux[:, 1:]-num_flux[:, :-1])/step
    # Preshock state Section 3.2.3 of [6]
    flux_der_preshock_2 = ( -2.*fluxes[:,end-6]+15.*fluxes[:,end-5]-60.*fluxes[:,end-4]
                           +20.*fluxes[:,end-3]+30.*fluxes[:,end-2]- 3.*fluxes[:,end-1])/60./step
    # Preshock state Section 3.2.3 of [6]
    flux_der_preshock_1 = ( -fluxes[:,end-5]+6.*fluxes[:,end-4]
                           -18.*fluxes[:,end-3]+10.*fluxes[:,end-2]+3.*fluxes[:,end-1])/12./step
    # Numerical conservative flux derivative for the nodes up to the shock
    flux_der = np.hstack((flux_der_for_domain, flux_der_preshock_2.reshape(-1,1), flux_der_preshock_1.reshape(-1,1)))
    dD_dt = D_RHS_approx_jit(gamma, D, step,
                             ex_fluxes[1, end-6:end], ambient, ambient_der, array,nodes,displacement, c_f, eps, k_for_fric)
...
@nb.njit('Tuple((f8[:,:], f8, f8,f8[:],f8))(i4, f8[:,:],f8[:], f8[:], f8[:], f8, f8, f8, f8, f8, f8, i4,i4,i4,i4, f8[:],f8,f8,f8,f8)')
def RK(Runge_Kutta, solution, heat_release, ambient, ambient_der, step, D_cur, gamma,
       act_energy, rate_const, CFL, start, finish, beg, end, nodes, displacement, c_f, eps, k_for_fric):
    """
    ...
    """
    SDO_approx, dD_dt, tau,c_fric = spatial_DiffOp_jit(solution, heat_release, ambient, ambient_der,
                                                 step, D_cur, gamma, act_energy, rate_const, CFL, start, finish-2,
                                                       end,nodes, displacement, c_f, eps, k_for_fric)
    update_solution = np.empty_like(solution)
    update_solution[:, start+1:finish] = solution[:,start+1:finish] + tau*SDO_approx
    D_next = D_cur + tau*dD_dt
    # Boundary conditions
    update_solution = Det_BC_WENO5_jit(update_solution, ambient, heat_release, D_next, gamma, beg, end)
    if Runge_Kutta == 3:
        """3rd order Runge-Kutta time discretization"""
        ### (Shu & Osher, 1988)
        update_solution2 = np.empty_like(solution)
        SDO_approx2, dD_dt2, _ ,c_fric2= spatial_DiffOp_jit(update_solution, heat_release, ambient, ambient_der,
                                                 step, D_next, gamma, act_energy, rate_const, CFL, start,
                                                            finish-2, end, nodes, displacement, c_f, eps, k_for_fric)
        D_1 = D_cur + tau*(.25*dD_dt + .25*dD_dt2)
        update_solution2[:, start+1:finish] = solution[:, start+1:finish]            + tau*(.25*SDO_approx + .25*SDO_approx2)
        update_solution2 = Det_BC_WENO5_jit(update_solution2, ambient, heat_release, D_1, gamma, beg, end)

        SDO_approx3, dD_dt3, _,c_fric3 = spatial_DiffOp_jit(update_solution2, heat_release, ambient, ambient_der,
                                                 step, D_1, gamma, act_energy, rate_const, CFL, start, finish-2,
                                                            end, nodes, displacement, c_f, eps, k_for_fric)
        D_next = D_cur + tau *             (1/6*dD_dt + 1/6*dD_dt2 + 2/3*dD_dt3)
        update_solution[:, start+1:finish] = solution[:, start+1:finish]            + tau*(1/6*SDO_approx + 1/6*SDO_approx2 + 2/3*SDO_approx3)
        update_solution = Det_BC_WENO5_jit(update_solution, ambient, heat_release, D_next, gamma, beg, end)
        D_acc = 1/6*dD_dt + 1/6*dD_dt2 + 2/3*dD_dt3
    if Runge_Kutta == 5:
        """5th order Runge-Kutta time discretization"""
        ### (Henrick, Aslam & Powers, 2006)
        j4_coefs = np.array([2046./15625., -454./15625. , 1533./15625. ])
        j5_coefs = np.array([-739./5625., 511./5625., -566./16875., 20./27.])
        j6_coefs = np.array([11822./21875., -6928./21875., -4269./21875., -4./7., 54./35.])
        b_coefs = np.array([1./24., 0.,  0., 125./336., 27./56., 5./48.])
        ...
        SDO_approx3, dD_dt3, _ ,c_fric3= spatial_DiffOp_jit(update_solution2, heat_release, ambient, ambient_der,
                                                 step, D_1, gamma, act_energy, rate_const, CFL, start,
                                                            finish-2, end, nodes, displacement, c_f, eps, k_for_fric)
        D_2 = D_cur + tau*dot_numba(j4_coefs,np.array([dD_dt, dD_dt2, dD_dt3]))
        update_solution2[:, start+1:finish] = solution[:, start+1:finish] +\
                tau*(j4_coefs[0]*SDO_approx+j4_coefs[1]*SDO_approx2+j4_coefs[2]*SDO_approx3)
        update_solution2 = Det_BC_WENO5_jit(update_solution2, ambient, heat_release, D_2, gamma, beg, end)

        SDO_approx4, dD_dt4, _ ,c_fric4= spatial_DiffOp_jit(update_solution2, heat_release, ambient, ambient_der,
                                                 step, D_2, gamma, act_energy, rate_const, CFL, start,
                                                            finish-2, end, nodes, displacement, c_f, eps, k_for_fric)
        D_3 = D_cur + tau*dot_numba(j5_coefs,np.array([dD_dt, dD_dt2, dD_dt3, dD_dt4]))
        update_solution2[:, start+1:finish] = solution[:, start+1:finish] +\
                tau*(j5_coefs[0]*SDO_approx + j5_coefs[1]*SDO_approx2 + j5_coefs[2]*SDO_approx3 + j5_coefs[3]*SDO_approx4)
        update_solution2 = Det_BC_WENO5_jit(update_solution2, ambient, heat_release, D_3, gamma, beg, end)

        SDO_approx5, dD_dt5, _ ,c_fric5= spatial_DiffOp_jit(update_solution2, heat_release, ambient, ambient_der,
                                                 step, D_3, gamma, act_energy, rate_const, CFL, start,
                                                            finish-2, end, nodes, displacement, c_f, eps, k_for_fric)
        D_4 = D_cur + tau*dot_numba(j6_coefs,np.array([dD_dt, dD_dt2, dD_dt3, dD_dt4, dD_dt5]))
        update_solution2[:, start+1:finish] = solution[:, start+1:finish] +\
                tau*(j6_coefs[0]*SDO_approx + j6_coefs[1]*SDO_approx2 + j6_coefs[2]*SDO_approx3
                        + j6_coefs[3]*SDO_approx4 + j6_coefs[4]*SDO_approx5)
        update_solution2 = Det_BC_WENO5_jit(update_solution2, ambient, heat_release, D_4, gamma, beg, end)

        SDO_approx6, dD_dt6, _,c_fric6 = spatial_DiffOp_jit(update_solution2, heat_release, ambient, ambient_der,
                                                 step, D_4, gamma, act_energy, rate_const, CFL, start, finish-2,
                                                            end, nodes, displacement, c_f, eps, k_for_fric)

        D_acc = dot_numba(b_coefs,np.array([dD_dt, dD_dt2, dD_dt3, dD_dt4, dD_dt5, dD_dt6]))
        D_next = D_cur + tau * D_acc
        update_solution[:, start+1:finish] = solution[:, start+1:finish] \
                + tau*(b_coefs[0]*SDO_approx + b_coefs[1]*SDO_approx2 + b_coefs[2]*SDO_approx3
                        + b_coefs[3]*SDO_approx4 + b_coefs[4]*SDO_approx5 + b_coefs[5]*SDO_approx6)
        update_solution = Det_BC_WENO5_jit(update_solution, ambient, heat_release, D_next, gamma, beg, end)
...
class Cell1D:
    ...
    """
    def __init__(self, xc=0., lenght=1., index=0):
        """
        Initializes a cell.
        ...
        # End points
        self.xl, self.xr = xc-.5*lenght, xc+.5*lenght
        # Initial and current values at xc
        self.init_val = 0.
        self.val = 0.
        # Flux of a cell
        self.flux = {index+1/2: 0.}
...
class Mesh1D:
    ...
    def __init__(self, x_0=0., x_N=1., num_nodes=2, num_ghost=0):
        """
        ...
        self.end = num_ghost + num_nodes  # numpy index next to the right endpoint + 1
        # domain = [beg:end]
        ...
        for i, xc in enumerate(np.linspace(x_0-self.NoG*self.step, x_N+self.NoG*self.step, self.total_num)):
    ...
    def set_values(self, init, with_averages=False, args=()):
        ...
        if callable(init):
            if with_averages:
                ...
                for i in range(self.total_num):
                    self.cells[i].val = init(self.cells[i].xc, *args).reshape((-1, 1))
            ...
            if (init.shape[1] == self.total_num):
                ...
                raise Exception(
                    "Input array shape: {}, when the mesh size is {}".format(init.shape[1],self.total_num))
        else:
            raise Exception(
    ...
    def plot_mesh(self, domain=True, with_fluxes=False):
        ...
        for cell in cells:
            ax.scatter(cell.xl, 0., color='b')
        ax.scatter(cells[-1].xr, 0., color='b', label='Cell borders')
        for cell in cells:
            ax.scatter(cell.xc, 0., color='r')
        ax.scatter(cells[-1].xc, 0., color='r', label='Cell centers')
#         if with_fluxes:
#             for cell in cells[1:]:
...
#                           0], 0, label='Cell fluxes')
    ...
    def plot_vals(self, exact=False, domain=True, labels=None, save=False):
        ...
        for i in range(values.shape[0]):
            ...
            if exact:
                ax.plot(self.get_nodes(domain=domain), values[i, :],
                        label=labels[i], color='blue')
            else:
                ax.scatter(self.get_nodes(domain=domain), values[i, :],
...
def calc_initial(file, L, N, all_nodes, NoG, gamma, Q, act_energy):
    ...
    """
    def for_reac_rate(y):
        D_CJ = np.sqrt(gamma + (gamma*gamma-1.)*Q/2.) +             np.sqrt((gamma*gamma-1.)*Q/2.)
        V_lam = gamma/D_CJ/D_CJ*(1.+D_CJ*D_CJ)/(gamma + 1.)*(1. -
                                                             (D_CJ*D_CJ-gamma)/(1.+D_CJ*D_CJ)/gamma*(np.sqrt(1-y)))
        u_lam = 1/(gamma + 1.)*(D_CJ*D_CJ-gamma)/D_CJ*(1+np.sqrt(1-y))
        p_lam = (1+D_CJ*
                 D_CJ)/(gamma + 1.) * \
            (1. + (D_CJ*D_CJ-gamma)/(1.+D_CJ*D_CJ)*(np.sqrt(1-y)))
        omega = (1-y)*np.exp(-act_energy/p_lam/V_lam)
        return (u_lam-D_CJ)/omega
    def RHS_lambda(x, y):
        rate_const, _ = integrate.quad(for_reac_rate, 0., .5)
        rate_const = -rate_const  # ???
        return rate_const/for_reac_rate(y)
    def ZND_init_cond(nodes):
        r = ode(RHS_lambda).set_integrator('zvode', method='bdf') # using pythonfunction
        ...
        cur_lam = 0.
        node_num = -2
        while (np.abs(cur_lam-1.) >= eps) and (node_num >= -nodes.shape[1]):
            z.append(r.t)
        ...
        lam_x = np.hstack(
            (np.ones(nodes.shape[1]-lam_x_raw.shape[1]).reshape((1, -1)), lam_x_raw))
        D_CJ = np.sqrt(gamma + (gamma*gamma-1.)*Q/2.) +             np.sqrt((gamma*gamma-1.)*Q/2.)
        V_x = gamma/D_CJ/D_CJ*(1+D_CJ*D_CJ)/(gamma + 1.)*(
            1. - (D_CJ*D_CJ-gamma)/(1.+D_CJ*D_CJ)/gamma*(np.sqrt(1-lam_x)))
        u_x = 1/(gamma + 1.)*(D_CJ*D_CJ-gamma) /             D_CJ*(1+np.sqrt(1-lam_x))
        p_x = (1+D_CJ*D_CJ)/(gamma + 1.) *             (1. + (D_CJ*D_CJ-gamma)/(1.+D_CJ*D_CJ)*(np.sqrt(1-lam_x)))
        # Rankine-Hugoniot conditions for variables at x=0
        (rho_a, u_a, p_a, lam_a) = (1.,0.,1.,0.)
        c_a = np.sqrt(gamma*p_a/rho_a)
        M_a = D_CJ/c_a
        p_s = (2.*gamma/(gamma+1.)*M_a*M_a -
               (gamma-1.)/(gamma+1.))*p_a
        rho_s = ((gamma+1.)*M_a*M_a /
                 (2.+(gamma-1.)*M_a*M_a))*rho_a
        u_s = (2./(gamma+1.)*(M_a*M_a-1.)/(M_a))*c_a
...
#             (rho_s, u_s, p_s, 0.)).reshape((-1, 1))
    ...
    if NoG:
        ...
        for i in range(NoG-1):
            ghost_array = np.hstack((ghost_array,
                                     init_array_non_positive[:, -1].reshape(-1, 1)))

        init_array = np.hstack((init_array_non_positive,
                                ghost_array))
...
#                     var_0 = var_a
        ...
        return np.array((var_a, 0.))

# @nb.njit('Tuple((f8[:], f8[:]))(f8,b1,f8[:,:,:],f8[:], f8[:], f8[:])')
def calculate_ambient_jit(xi, upstream, table, ambient, wave_numbers, var_upstream_A):
    if not upstream:
        # ambient state
        return ambient, np.array([0.,0.,0.,0.])
    else:
        # ambient periodic state
        rho_a, rho_der = var_upstream_jit(xi, 0,table, ambient, wave_numbers, var_upstream_A)
        u_a, u_der = var_upstream_jit(xi, 1,table, ambient, wave_numbers, var_upstream_A)
        p_a, p_der = var_upstream_jit(xi, 2,table, ambient, wave_numbers, var_upstream_A)
        lambda_a, lam_der = var_upstream_jit(xi, 3,table, ambient, wave_numbers, var_upstream_A)
    return np.array((rho_a, u_a,p_a,lambda_a)), np.array((rho_der, u_der,p_der,lam_der))
...
class Simulation:
    def __init__(self,
                 ...
                 Q_wave_number = 0.,
                 Q_amp = 0.,
                 wave_numbers = (0., 0., 0., 0.),
                 var_upstream_A = (0., 0., 0., 0.),
                 store = True,
                 c_f = 0.,
                 eps = 0.,
                 k_for_fric = 0.):
        ...
        self.names = ('Density', 'Velocity',
                      'Pressure', 'Reaction progress')
        self.num_names = ('num_Density', 'num_Velocity',
                          'num_Pressure', 'num_Reaction progress')
        ...
        nu = 1.               # exponent the reaction rate
        gamma = 1.2           # specific heat ratio
        Q = 50.               # average heat release
        self.shock_speed = [] # to write D values
        self.shock_acc = []   # to write dD/dt values
        self.dist = [0.]      # to write the shock position in the lab frame
        # Constant ambient state
        self.ambient = np.array([1., 0., 1., 0.])
        ...
        if act_energy:
            ...
            self.attrs = {'time_limit':time_limit, 'final_dist':None, 'final_time_step':None}

            # Define the mesh and initial conditions
            my_mesh = Mesh1D(-L, 0., num_nodes=N, num_ghost=3)
...
#             ones = np.ones((1, self.NoG))
            ...
            with h5py.File('initials.hdf5', 'a') as file:
                if f'E{act_energy}Q{Q}'+self.group_name in file:
                    ...
                    self.rate_const, init_array = calc_initial(file, L, N, self.nodes,
                                                               self.NoG, gamma, Q, act_energy)
        ...
        if np.sum(wave_numbers):
            self.upstream = True
            self.min_wn=np.min(wave_numbers[wave_numbers>0.])
        else:
            self.upstream = False
        ...
        def nonuniform_heat(array):
            ...
            """
            if (array>1/Q_wave_number).all(): # здесь ты просчитываешь в каждой точке свою периодич функцию.
                return Q*(1. + Q_amp*(np.sin(2.*np.pi*Q_wave_number*array)))
            else:
                ...
                for i, x in enumerate(array[0,:]):
                    if x<=0.:
                        ...
                        heat_array[:,i] = Q*(1. + Q_amp*(np.sin(2.*np.pi*Q_wave_number*x) ))
                return heat_array

        def heat_release_func(array_in, x=0.):
            """
            ...
            if array_in.shape[1] == my_mesh.NoN + 2*my_mesh.NoG:
                ...
                array = array_in[:, my_mesh.end -
                                   array_in.shape[1]:my_mesh.end]
            if Q_wave_number:
                array_mov = array + x
        ...
        def variable_transform(array, toConserved=False):
            ...
            """
            return variable_transform_jit(array, self.heat_release, gamma, toConserved)

        my_mesh.set_values(init_array)
        self.init_cond_phys = my_mesh.get_values(domain=True)
        # Turn variables into the conserved form
        my_mesh.set_values(variable_transform(
            my_mesh.get_values(), toConserved=True))
    ...
    #                         var_0+upstream_A*np.sin(2.*np.pi*wave_number*x_for_periodic))
                        ...
                        value_der = 0 if value==0 else interpolate.splev(x, tck, der=1)
                        return np.array((value, value_der))
                    else:
    #                     value = max((0, var_0+upstream_A*np.sin(2.*np.pi*wave_number*x)))
    #                     value_der = 0 if value==0 else 2.*np.pi*wave_number*upstream_A*np.cos(2.*np.pi*wave_number*x)
                        # For temperature
                        value = max((0, 1./(1. + upstream_A*(np.cos(2.*np.pi*wave_number*x) - 1.))))
                        value_der = 0 if value==0 else 2.*np.pi*wave_number*upstream_A*np.sin(2.*np.pi*wave_number*x)*value**2
                        return np.array((value, value_der))
                if k == 3:
    ...
    #                         var_0+upstream_A*np.sin(2.*np.pi*wave_number*x_for_periodic))
                        y_for_periodic = np.array(
                            1.-np.exp(upstream_A*(np.cos(2*np.pi*wave_number*x_for_periodic)-1)))
                        y_for_periodic = np.where(
                            y_for_periodic >= 0, y_for_periodic, 0)
                        x_for_spline = np.hstack([x_for_const, x_for_periodic])
                        y_for_spline = np.hstack([y_for_const, y_for_periodic])
    ...
    #                     value_der = 0 if value==0 else 2.*np.pi*wave_number*upstream_A*np.cos(2.*np.pi*wave_number*x)
                        # For temperature
                        value = max((0, 1. - np.exp(upstream_A*(np.cos(2*np.pi*wave_number*x)-1))))
                        value_der = 0 if value==0 else 2.*np.pi*wave_number*upstream_A*np.sin(2.*np.pi*wave_number*x)*(1. - value)
                        return np.array((value, value_der))
            else:
                return np.array((var_a, 0.))
        ...
        if not only_init:
           ...
           # t = tqdm.tqdm(range(int(time_limit)), desc='Time integration')
            while self.time<time_limit:
                ...
                # RK 3-5 begins
                solution, D_next, self.tau,c_fric, D_acc = RK(Runge_Kutta, solution, self.heat_release, ambient,
                                                 ambient_der, self.step, D_cur, gamma, self.act_energy,
                                                 self.rate_const, self.CFL, start, finish, beg,
                                                    end,self.nodes.reshape(1, -1)[0], xi,
                                                    c_f, eps, k_for_fric)
                ...
                self.dist.append(self.dist[-1] +
                                     self.tau*D_cur)
                self.heat_release = heat_release_func(self.nodes.reshape(1, -1), x=self.dist[-1])
            ...
            #print(check)
            #t.close()
            results = np.vstack((np.array(self.times),np.array(self.shock_speed),np.array(self.shock_acc)))
        ...
        my_mesh.set_values(variable_transform(
            conserved_solution, toConserved=False))
        ...
        if (not only_init) and store:
            with h5py.File(self.file_name+'.hdf5', 'a') as file:
                if self.group_name+'/'+self.data_name in file:
                    ...
                    group.attrs.modify('final_time_step',self.attrs['final_time_step'])
                    ...
                    if np.isnan(results).any() or np.isnan(physical_solution).any():
                        print('There are NaNs in the solution. No data saved')
                    else:
                        group = file.create_group(self.group_name+'/'+self.data_name)
                        group.attrs.update(self.attrs)
                        dset = group.create_dataset(
                            'Shock speed', data = results, maxshape=(results.shape[0],None), compression='lzf', fletcher32=True)
                        # dset = group.create_dataset(
...
# Catching command line arguments using argparse
 ...
 #   parser.add_argument('amp', type=float,
 #                       help='A required amplitude of the periodicity')
    parser.add_argument('c_f', type=float,
                        help='A required float coefficient of friction')
    parser.add_argument('eps', type=float,
                        help='A required eps (coefficient in front of the sine )')
    parser.add_argument('k_for_fric_start', type=float,
                        help='A required starting period of friction')
    parser.add_argument('k_for_fric_end', type=float,
                        help='A required ending period of friction')
    parser.add_argument('k_for_fric_step', type=float,
                        help='A required step for period of friction')
    args = parser.parse_args()
    print(f'Run simulation from k_for_fr={args.k_for_fric_start} to k_for_fr={args.k_for_fric_end} with the step {args.k_for_fric_step}')

    k_for_frics = np.round(np.arange(args.k_for_fric_start, args.k_for_fric_end+args.k_for_fric_step/2., args.k_for_fric_step),6)
    ...
    for k_for_fr in k_for_frics:
        ...
        try:
            ...
            amp = args.eps
            Simulation(act_energy=args.act_energy, Runge_Kutta=3,
                    L = args.dom_length, N=args.num_nodes, time_limit=2000,
                    wave_numbers=(0.0,0.0,0.0,0.0),var_upstream_A=(0.0,0.0,0.0,0.0),
                    c_f=args.c_f, eps=amp, k_for_fric=k_for_fr, CFL=0.8)
