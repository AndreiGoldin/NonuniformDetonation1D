...
out_dir="/gss/home/a.goldin/Detonation/periodicFric"
...
with open(parameter_file_full_path, "r") as csvfile:
    ...
    for job in reader:
        ...
        first_part = f"qsub -v ENERGY={job[0]},LENGHT={job[1]},NODES={job[2]},CF={job[3]},EPS={job[4]},START={job[5]},"
        second_part = f"FINISH={job[6]},STEP={job[7]},OUT_DIR={out_dir}"
        third_part = f" -N E{job[0]}L{job[1]}N{job[2]}c_f{job[3]}eps{job[4]}_from{job[5]}to{job[6]}step{job[7]}"
